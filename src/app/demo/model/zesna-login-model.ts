...
import { ZesnaLoginService } from "../service/zesna-services/zesna-login.service";
...
export class ZesnaLoginModel{
    ...
    LogoutUserService(email: string) {
     ...
     CheckEmailExistsService(userEmail: string) {
        var promise = new Promise((resolve, reject) => {
            this.allSubscriptions.push(this._zesnaLoginService.CheckEmailExists(userEmail).subscribe(
                data => {
                    let returnData = <boolean>data;
                    // Resolve the promise
                    resolve(returnData);
